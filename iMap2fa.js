...
async function getLatestVerificationCodeFromEmail({
                                                      pollIntervalMs = 2500,
                                                      pollTimeoutMs = 90000,
                                                      fromCandidates = ['nextdoor', 'no-reply@nextdoor.com', 'hello@nextdoor.com', 'hs.email.nextdoor.com'],
                                                  } = {}) {
    ...
    const boxesToTry = ['Spam'];
    let boxOpened = false;

    for (const box of boxesToTry) {
        try {
            console.log(`📬 Attempting mailbox: ${box}`);
    ...
    try {
        while (Date.now() - start < pollTimeoutMs) {
            const results = await connection.search(
                ['ALL', ['SINCE', new Date(Date.now() - 1000 * 60 * 60)]],
            ...
            for (let i = results.length - 1; i >= 0; i--) {
                ...
                const m = text.replace(/\D+/g, '').match(/(\d{6})/);
